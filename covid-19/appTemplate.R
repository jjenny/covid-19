...
##policytext
policy_choices = c("Don't show", "Reopen", "Lockdown", "Schools closed")
policy_text = c("-", "Reopen                 ", "Lockdown                   ", "Schools closed                          ")

norm_choices = c("Nothing", "Population (thousands)", "Total tests performed")
norm_text =  c( "Confirmed cases", "Confirmed cases per thousand people", "% Confirmed cases per total tests")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectizeInput("states1", "States (over #CUTOFF_2# cases):", 
                     states$bin1,
                     multiple=TRUE, 
                     select = states$bin1growing),
      selectizeInput("states2", "States (over #CUTOFF_1# cases):", 
                     states$bin2, 
                     multiple=TRUE,
                     select = states$bin2growing),
      selectizeInput("states3", "States (over #CUTOFF_0# cases):", 
                     states$bin3, 
                     multiple=TRUE,
